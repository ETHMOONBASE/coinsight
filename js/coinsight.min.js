...
(function(b, a) {
 ...
 var k = Array.isArray || function(x) {
  return u.call(x) === "[object Array]"
 };
 ...
 function m(x) {
  return String(x).replace(/[&<>"'\/]/g, function(y) {
   return d[y]
  })
 }
 var f = /\s*/;
 ...
 function w(Q, F) {
  ...
  while (!z.eos()) {
   ...
   if (y === "#" || y === "^") {
    ...
    if (y === "/") {
     x = H.pop();
     if (!x) {
      throw new Error('Unopened section "' + I + '" at ' + A)
     }
     if (x[1] !== I) {
      throw new Error('Unclosed section "' + x[1] + '" at ' + A)
     }
    } else {
     if (y === "name" || y === "{" || y === "&") {
      O = true
  ...
  x = H.pop();
  if (x) {
   throw new Error('Unclosed section "' + x[1] + '" at ' + z.pos)
  }
  return v(c(G))
 }
 ...
 s.prototype.scanUntil = function(z) {
  var y = this.tail.search(z),
   x;
  switch (y) {
   case -1:
    x = this.tail;
 ...
 p.prototype.push = function(x) {
  return new p(x, this)
 };
 p.prototype.lookup = function(z) {
  var x = this.cache;
  var B;
  if (z in x) {
   B = x[z]
 ...
 o.prototype.parse = function(z, y) {
  var x = this.cache;
  var A = x[z];
  if (A == null) {
   A = x[z] = w(z, y)
 ...
 o.prototype.render = function(A, x, z) {
  var B = this.parse(A);
  var y = (x instanceof p) ? x : new p(x);
  return this.renderTokens(B, y, z, A)
 };
 o.prototype.renderTokens = function(F, x, D, I) {
  var B = "";
  var J = this;

  function y(K) {
   return J.render(K, x, D)
  }
  var z, G;
  for (var C = 0, E = F.length; C < E; ++C) {
   ...
   switch (z[0]) {
    case "#":
     ...
     if (k(G)) {
      for (var A = 0, H = G.length; A < H; ++A) {
       B += this.renderTokens(z[4], x.push(G[A]), D, I)
      }
     } else {
      if (typeof G === "object" || typeof G === "string") {
       B += this.renderTokens(z[4], x.push(G), D, I)
      } else {
       if (b(G)) {
        if (typeof I !== "string") {
    ...
    case ">":
     ...
     G = b(D) ? D(z[1]) : D[z[1]];
     if (G != null) {
      B += this.renderTokens(this.parse(G), x, D, G)
     }
     break;
    case "&":
     G = x.lookup(z[1]);
 ...
 a.parse = function(y, x) {
  return i.parse(y, x)
 };
 a.render = function(z, x, y) {
  return i.render(z, x, y)
 };
 a.to_html = function(A, y, z, B) {
  var x = a.render(A, y, z);
  if (b(B)) {
   B(x)
  } else {
   return x
...
function resize_charts() {
 if (jQuery.browser.mobile || $(window).width() <= 800) {
  $(".half-chart").css("width", "100%")
 } else {
  if ($(window).width() >= 3200) {
   $(".half-chart").css("width", "24.75%")
  } else {
   $(".half-chart").css("width", "49.5%")
  }
 }
}
